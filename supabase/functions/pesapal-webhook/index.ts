...
serve(async (req) => {
  ...
  try {
    ...
    // 10. Send confirmation eTicket email. Wrapped in EdgeRuntime.waitUntil
    // so the fetch survives the response (Supabase Edge Runtime aborts
    // in-flight fetches when the handler returns). Pesapal still gets a fast
    // IPN ack because waitUntil doesn't block the response.
    //
    // The payload contains everything send-confirmation needs to render an
    // eTicket-quality email: the full Duffel order (for slices, segments,
    // documents, passengers, conditions), our stored seat/bag selections
    // (for assignments per passenger), and a derived KES fare breakdown.
    //
    // Note: `storedSeats` and `storedBaggages` are already declared higher up
    // (around line 299) for the Duffel order construction — reuse them here.
    ...
    const sendEmailPromise = fetch(SEND_CONFIRMATION_URL, {
      ...
      body: JSON.stringify({
        ...
        order, // full Duffel order — has slices, segments, documents, conditions
